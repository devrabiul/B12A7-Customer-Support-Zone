...
function App() {
  ...
  const [ticketsJson] = useState(ticketJsonData);
  const [resolvedTasks, setResolvedTasks] = useState([]);

  const handleSelectTicket = (ticket) => {
    if (!tasks.find((t) => t.id === ticket.id)) {
      setTasks([...tasks, ticket]);
      toast(`Ticket added to Task Status Section`);
    }
  };

  const handleComplete = (ticket) => {
    toast(`Ticket marked as complete`);
  ...
  for (let i = 0; i < ticketsJson.length; i++) {
    if (ticketsJson[i].status === "In- Progress") {
      ...
      <div className="container mx-auto px-[1rem] pb-[80px]">
        <div className="grid grid-cols-12 gap-[32px]">
          ...
          <div className="col-span-3">
            <TaskStatusSection tasks={tasks} resolvedTasks={resolvedTasks} onComplete={handleComplete} />
          </div>
        </div>
      </div>
